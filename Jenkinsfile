  ...
  stages {
    ...
    stage('Release docker image for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          feature_name=`echo $BRANCH_NAME | awk -F '/' '{ print $2 }'`
          set +e
          docker images | grep chain-gateway | grep $feature_name
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          set +e
          docker images | grep chain-gateway | grep latest
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
    ...
    stage('Deploy for feature') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          feature_name=`echo $BRANCH_NAME | awk -F '/' '{ print $2 }'`
          sed -i "s/chain-gateway:latest/chain-gateway:$feature_name/g" cmd/chain-gateway/k8s/02-chain-gateway.yaml
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh 'sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/chain-gateway/k8s/02-chain-gateway.yaml'
